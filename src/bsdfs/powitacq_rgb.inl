...
measured::measured(const std::string &filename) {

    Tensor tf = Tensor(filename);
    auto& diffuse = tf.field("diffuse");
    auto& roughness = tf.field("roughness");
 //   auto& specular = tf.field("specular");
    auto& phi_i = tf.field("phi_in");
    auto& theta_i = tf.field("theta_in");
    auto& rgb = tf.field("measure_brdf");
    ...
    if (!(theta_i.shape.size() == 1 &&
          ...
          diffuse.shape.size() == 1 &&
          diffuse.dtype == Tensor::Float32 &&
        //  specular.shape.size() == 1 &&
       //   specular.dtype == Tensor::Float32 &&

          roughness.shape.size() == 1 &&
          roughness.dtype == Tensor::Float32 &&
    ...
    m_data = std::unique_ptr<measured::Data>(new measured::Data());

    std::vector<float> diffuse_data(diffuse.shape[0]);
    std::vector<float> roughness_data(roughness.shape[0]);
  //  std::vector<float> specular_data(specular.shape[0]);
    const float *diffuse_values = (const float *) diffuse.data.get();
    const float *roughness_values =(const float *) roughness.data.get();
  //  const float *specular_values =(const float *) specular.data.get();
    memcpy(diffuse_data.data(), diffuse_values,
                   sizeof(float) * diffuse.shape[0]);
    memcpy(roughness_data.data(), roughness_values,
                   sizeof(float) * roughness.shape[0]);
  //  memcpy(specular_data.data(), specular_values,
  //                 sizeof(float) * specular.shape[0]);               
   measured::m_diffuse = Vector3f(diffuse_data[0], diffuse_data[1], diffuse_data[2]);
    measured::m_roughness = roughness_data[0];
 //   measured::m_specular = specular_data[0];                               
...
Vector3f measured::eval(const Vector3f &wi, const Vector3f &wo,Vector2f &pos) const {
    if (wi.z() <= 0 || wo.z() <= 0)
        return Vector3f(0.f);
   ...
   //   float theta_i = std::acos(wi.z()),
    ...
    Vector3f fr = Vector3f(0.f);
    for (int i = 0; i < 3; ++i) {
        float params_fr[3] = {phi_i, theta_i, float(i) };

        fr[i] = m_data->rgb.eval(pos, params_fr);
