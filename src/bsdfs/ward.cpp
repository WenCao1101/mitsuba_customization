...
class Ward final : public BSDF<Float, Spectrum> {
public:
    MI_IMPORT_BASE(BSDF, m_flags, m_components)
    MI_IMPORT_TYPES(Texture)

    Ward(const Properties &props) : Base(props) {
     ...
     //   m_eta = int_ior / ext_ior;
        ...
        m_alpha = props.texture<Texture>("alpha", .1f);
    ...
    std::pair<BSDFSample3f, Spectrum> sample(const BSDFContext &ctx,
       ...
       //                         MI_ROUGH_TRANSMITTANCE_RES, active);
        ...
        if (dr::any_or<true>(sample_specular)) {
            ...
            */
            Float  alpha = m_alpha->mean();
             Float phiH=2.0f*dr::Pi<Float>*sample2.y();  
             phiH= dr::select(sample2.y()>0.5,phiH+dr::Pi<Float>,phiH);
            
             Float thetaH=dr::atan(alpha*dr::sqrt(-dr::log(sample2.x()))); 
             Vector3f m=Vector3f(dr::sin(thetaH)*dr::cos(phiH),dr::sin(thetaH)*dr::sin(phiH),dr::cos(thetaH));   
             Vector3f wo=m*2.0f*dr::dot(si.wi,m)-si.wi;
    ...
    Spectrum eval(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        if (has_specular) {
            ...
            Vector3f H = dr::normalize(wo + si.wi);
            Float  alpha = m_alpha->mean();
            Float factor1=1.0f/(4.0f*dr::Pi<Float> *alpha*alpha* dr::sqrt(Frame3f::cos_theta(wo)*Frame3f::cos_theta(si.wi)));
            Float tan_thetaH=Frame3f::tan_theta(H);
            Float exponent=-(tan_thetaH*tan_thetaH)/(alpha*alpha);
            Float specRef=factor1*dr::exp(exponent);



            if (m_specular_reflectance)
                value = m_specular_reflectance->eval(si, active)*specRef;
        }
        ...
        //    value += diff * (dr::InvPi<Float> * m_inv_eta_2 * cos_theta_o * t_i * t_o);
               value +=diff*dr::InvPi<Float>;
        }
            value=value*Frame3f::cos_theta(wo);
    ...
    Float pdf(const BSDFContext &ctx, const SurfaceInteraction3f &si,
     ...
     //    Float result = 0.f;
         Float  alpha = m_alpha->mean();
         Float factor1=1.0f/(4.0f*dr::Pi<Float> *alpha*alpha* dr::dot(H,si.wi)*dr::pow(Frame3f::cos_theta(H),3));
         Float exponent=-(Frame3f::tan_theta(H)*Frame3f::tan_theta(H))/(alpha*alpha);
         Float specPdf=factor1*dr::exp(exponent);
         result=prob_specular*specPdf;
...
MI_IMPLEMENT_CLASS_VARIANT(Ward, BSDF);
MI_EXPORT_PLUGIN(Ward, "Ward");
